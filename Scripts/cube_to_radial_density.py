...
def load_cube_density(filename):
    ...
    # Count atoms from line 2
    num_atoms = abs(int(lines[2].split()[0]))
    start_line = 6 + num_atoms + 1

    # Parse float values, limiting to total_points
    raw_floats = []
    ...
    if len(raw_floats) < total_points:
        raise ValueError(f"Expected {total_points} data points, got {len(raw_floats)}")

    data = np.array([float(val) for val in raw_floats[:total_points]])

    # Build 3D grid and radial distances
...
if __name__ == "__main__":
    ...
    np.savetxt("radial_density.csv", np.column_stack((r_bins, rho_r)), delimiter=',', header='r,rho(r)', comments='')

    plt.plot(r_bins, rho_r)
    plt.xlabel("Radius r (Bohr)")
    plt.ylabel("Radial Density ρ(r)")
    plt.title("Spherically Averaged Electron Density")
    plt.grid(True)
